...
class GameState:
    ...
    def update_last_word(self, word, reason, similarity):
        self.last_word = word
...
def get_word_frequency_score(word):
    length_score = LENGTH_SCORE_WEIGHT / (len(word) + 1)
    common_word_score = COMMON_WORD_SCORE if word.lower() in COMMON_WORDS else 0
...
def get_related_word_with_reason(word):
    ...
    for synset in synsets:
        for hypernym in synset.hypernyms():
            ...
            score = get_word_frequency_score(word) * DIRECT_RELATION_BOOST
    ...
    valid_words.sort(key=lambda x: (
        x['relation_type'] in ['hypernym', 'hyponym'], x['score']
    ), reverse=True)
...
def format_response(message):
    ...
    if message == "how":
        if game_state.last_word:
            contextual_def = get_contextual_definition(
                game_state.last_word, game_state.last_word, game_state.last_reason
            )
    ...
    if game_state.last_word:
        if message == game_state.last_word.lower():
    ...
    game_state.update_last_word(best_related['word'], best_related['reason'], best_similarity)
