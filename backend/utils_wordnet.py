...
def get_trained_relations(word, train_of_thought=[]):
    ...
    for key, data in pairs.items():
        if word.lower() in [data['word1'].lower(), data['word2'].lower()]:
            ...
            if rating_score > 0 or has_sentences:                related_words.append({
                    'word': related_word,
...
def get_wordnet_relations(word, train_of_thought=[]):
    ...
    for synset in synsets:
        ...
        for w in synonyms:
            if w != word:                related_words.append({
                    'word': w,
        ...
        for w in hyponyms:            related_words.append({
                'word': w,
        ...
        for w in hypernyms:            related_words.append({
                'word': w,
