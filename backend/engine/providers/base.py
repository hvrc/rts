...
A provider is a brain. It gets a rendered system prompt, the turn's user message, and
a read-only view of the game (`ctx`), and returns a dict matching schema.MOVE_SCHEMA.
That's the entire contract — the engine never knows or cares which model answered.
